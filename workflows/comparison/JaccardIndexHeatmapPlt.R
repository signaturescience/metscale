...
out_file <- paste0(substr(file_name, 1, nchar(file_name) - 4), ".png")
png(file = out_file, height = 8.5, width = 8.5, units = "in", res = 250)
p <- ggplot(data = plot_df, mapping = aes(x = Sample.x, y = Sample.y, fill = Jaccard.Index, label = round(Jaccard.Index, 2))
